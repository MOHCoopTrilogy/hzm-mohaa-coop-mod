> Generated 2026-09-13 by the mp-progression-understand workflow (28 agents: 7 readers, 7 adversarial verifiers, a critic, gap readers + verifiers, synthesis). Research only; it designs nothing. It follows the user's MP progression decisions recorded at the top of mp_loadout_plan_v1.md. The synthesis was emitted in 3 parts; this file joins them.
...
1. Which binaries may MP progression change: game.dll (required anyway), cgame.dll, openmohaa.exe? This picks the store and carrier (section 10).
2. Is it acceptable that a hostile server can erase, but not forge, MP progress? If not, the trampoline class must be closed in the exe.
3. File the seta/vstr and cfg-trampoline arbitrary-command hole now, as a coop-wide security bug? It affects every HZM client today.
4. T1 only, or plan a T2 trusted-issuer tier later? T2 closes key extraction and listen-host minting, but costs key management.
5. Do listen-server-earned records count everywhere, or get tagged so dedicated operators can refuse them? Listen hosts can mint.
6. Are unlocks latched forever, or re-derived on each load? Re-deriving re-locks gear after any retune.
7. After a class-map change, do past kills stay in the ladder they were counted under? Freezing class at kill time says yes.
8. MP class vocabulary: split tab 3 into MG, SHOTGUN and ROCKET? Where do StG44 Scoped and the SVT/G43 zoom rifles go? This defines the class counters.
9. Is a headshot location 0-1 or 0-2? The engine groups 0-2; changing it later changes challenge meaning.
10. Does a variant or skin kill advance the family counter, the variant's own counter, or both? Is a scoped Garand kill SNIPER? This sets the per-source counter semantics.
11. Does a kill with a picked-up enemy weapon advance that weapon's counter? Grenade pickups are common.
12. Do melee kills count toward the held gun's class? That affects ladder speed.
13. Do stock non-roster kit items (kar98_mortar, *_lite rifles, F1, mine detectors) earn anything while the stock kit is live? It decides the reserved source ids.
14. Is one starter gun per class free? The pre-verification policy, fail-closed mode, stock clients and bots all assume yes. And is the starter a table policy, or latched into the record?
15. Bot-farming policy: per-map or per-hour cap, weighting, zero credit under bot_manualmove or sv_cheats, or require a second human? Decision E makes bot kills count.
16. Dress bots from the MP free tier, or leave their random coop skins? Visible locked gear otherwise.
17. Japanese character skins do not exist. Drop Japanese from cosmetics (keep the weapons), or commission models?
18. Must voice nationality match the worn MP skin? That needs an engine override, not script.
19. Should two people on one Windows account keep separate progress? Only a per-person homepath solves it.
20. Offer a "back up MP progress" export of qkey plus the record? It is the only recovery after qkey loss.
21. Accept losing credit after the last echo when a player leaves or a server crashes, or allow a server-side pending journal file (a narrow exception to D)?
22. After a late verification, apply the wanted kit mid-life or at the next spawn? Mid-life needs takeall+give.
23. After a failed verification, is session credit discarded, or written into the client record? Writing it risks laundering.
24. Must the MP armories work from the disconnected main menu with padlocks? That needs client-held state and adds MAX_CVARS pressure.
25. Is an offline MP Service Record or pins wanted? Both are engine-coupled to coop cvar names.
26. Are shared assets (sound aliases, fill shaders) acceptable under F? They are neither code nor state.
27. Existing mp.scr couplings (statefile, player.scr/main.scr labels, monitors): allowlist or replace? They are live F exposure today.
28. May coop be protected from operator-set bots entering coop maps? Any protection touches coop files.
29. Are game.dll delegates for intermission, teamwin and TOW takeover acceptable? Match-win and exact objective credit depend on them.
30. Drop accuracy and shots-fired challenges from the first slice? No script source exists.
31. Any perk-like rewards in MP? 15 coop perk_* rewards have no armory item.

### (b) Playtests (cheapest test)

1. PvP damage gate: listen TDM, addbot, shoot it, check health and obituary. BLOCKER for everything kill-based.
2. `println (md5string "abc")` twice on the live game.dll. Expected 900150983cd24fb0d6963f7d28e17f72; decides whether md5string is usable at all.
3. event_subscribe unquoted file::label stub printing 11 args and self (K1/K2). The whole kill feed rests on it.
4. Kill-attribution matrix K3-K19 from 4.5: validates every expression in 4.3.
5. Dedicated server plus remote client: `seta coop_mpq0 coopprof 0 HZMPROBE_A`, server stuffs `vstr coop_mpq0`, print coop_profdata. Repeat with a 300-char chunk, then with sv_floodPro8. **omconfig.cfg archive (client disk).** Lines are written quoted (cvar.c:1256/1262). A line whose name+value+10 exceeds 1024 is skipped with only a yellow warning; the live value stays in memory and is lost at quit (cvar.c:1237-1266) [C]. Fully covered in section 2.8. The live config already re-creates 3,951 `seta coop_` cvars at startup (123,674 bytes) [M].

9. **New CG_ServerCommand handler (S->C).** Must live inside CG_ServerCommand, not be registered with cgi.AddCommand: console text and trampoline expansions reach only CG_ConsoleCommand, so they cannot invoke it (cg_servercmds.c:386-530; cl_cgame.cpp:1003-1008) [NV]. Any server can still send it, so cgame must merge monotonically. Needs cgame.dll.

10. **cgame file write (client gamedir).** cgi.FS_ReadFile / FS_WriteFile exist (cg_public.h:140-143) [NV]. cgame has no FS_SV_* and no rename, so it cannot write at the homepath root (cg_public.h:140-145) [NV].

11. **New exe file at the homepath root.** FS_SV_* writes there, next to qkey; the location does not change with the target game (files.cpp:803, :857) [NV]. Only new exe code can write it atomically (section 2.7).

12. **Server file (fs_write_content).** Server disk only. Truncate-then-write, no temp file (scriptthread.cpp:7400-7405; files.cpp:2068-2091) [NV]. Ledger use only, not a portable carrier.

13. **Server cvar.** Survives a map change and a round restart (challenges.scr "session cvar first" precedent, :815-823) [P]. getcvar mutates the value: if the string contains a '.', it strips trailing '0' then a trailing '.' ("v1.50" -> "v1.5") (scriptthread.cpp:2630-2641) [P]. A serialized blob in a cvar must not end in '0' when it contains a '.'.

14. **Chat + player_textMessage filter.** Needs event_subscribe and pollutes chat (player.cpp:12831-12835) [S]. Reject.

**Unsafe characters (any layer).**

- Cvar value: backslash, double quote, semicolon refuse the set (cvar.c:81-96, :616-625) [NV].
- Userinfo key or value: backslash, semicolon, double quote refuse the pair; a pair that would reach 1350 is silently skipped (q_shared.c:1950-1980) [NV].
- Stufftext payload: double quote ends the argument (player.cpp:12381, bug-758); ';' and newline split statements; `//` and `/*` outside quotes start comments; whitespace runs collapse (cmd.c:219-252, :680-757) [NV].
- coopprof arguments: a space splits into ignored extra args; `%` becomes `.` (msg.cpp:700-702); `\n`/`\r` become spaces; over 255 bytes truncates [NV].
- Recommended alphabet: `[A-Za-z0-9-_]` (6 bits/char). If runtime case sensitivity fails, fall back to base-32 `[0-9a-v]` (5 bits/char, x1.2 size). Case sensitivity is code-confirmed but not runtime-tested (scriptvariable.cpp:2027-2062; con_set.cpp:33-43) [NV].

### 2.3 Capacity math

64-symbol alphabet, fixed-width fields, from the persistence report's scratch encoder [NV/M]:

- Class counters: 7 x 4 chars = 28 (0..16,777,215 each).
- Weapon counters: 80 x 3 chars = 240; at 4 chars = 320.
- Challenge completion bits: 400 -> 67 chars.
- Header: version + 16-char id + 4-char seq + 3-char length + 32-hex MAC = 56 (or 40 with a 16-hex MAC).
- Core record (3-char weapons, 400 flags, no unlock bits): 391 chars.
- Max core (4-char weapons + 600 unlock bits): 571 chars.
- Dense in-progress challenge counters push it toward 821, then 1771.

The class-mapping report's fuller catalogue (both armories, unlocks kept as tokens) gives a scratch worst case of 4,983 bytes (20 chunks) and a typical player about 1,063 bytes (5 chunks) [M]. Both fit under the 8192 coopprof cap. **No bitmap is needed, and none should be used, because a positional bitmap collides with the "persist ids, never positions" rule (section 5.4).**

Chunking: `ceil(len/255)` sends. 391 = 2 chunks, 571 = 3, 4,983 = 20. Under sv_floodProtect 1, pace at >= 1.1 s per chunk, so the 5 KB worst case is roughly 22 s.

Write-back: `ceil(len/255)` stufftexts, each about 280 bytes on the wire, each persisted value about 267 bytes (well under the 1014-len cap).

### 2.4 Remote / dedicated / stock-client behaviour

- **Listen host proves nothing about remote clients.** A command the host runs is consumed by G_ConsoleCommand as client 0, bypassing the 255-byte cap and flood protection (cmd.c:1052-1064; sv_game.c:1962-1967) [NV LB]. Every remote limit must be tested on a dedicated server with a second client.
- **Flood protection.** Default 0 (sv_init.c:1114), but the mod's own server cfgs set it to 1 (net_sv.cfg:16; coop_mod/cfg/dedicated_example.cfg:33) [C]. When on, any client game command within 1000 ms of the previous one is dropped, and userinfo changes re-arm the timer [C].
- **Stock OpenMOHAA client.** Upstream filter (ea8ee677) drops seta coop_*, vstr and exec. Such a client stores nothing and cannot be told to send [NV LB]. The MP armory must degrade a stock client to the free starter kit per class.

### 2.5 Recommended transport, both directions

- **Picks and small acks, C->S: registered userinfo keys.** State, not events. Present at spawn, exempt from flood protection, and it survives restart and map change. Needs about two lines in g_client.cpp plus client registration (like coop_pin1..5). See section 5 for the ack contract.
- **Record body upload, C->S: a dedicated MP EV_CONSOLE event, chunked.** Never share coopprof with picks: idx 0 wipes the one buffer [C]. 255-byte chunks, index 0 first, paced >= 1.1 s when sv_floodProtect is set.
- **Record write-back, S->C: a dedicated CG_ServerCommand handler that merges monotonically.** Not stufftext of coop_*, which any server can forge or wipe.
- **Split rule.** Header (magic, version, portable id, seq, length, chunk count, MAC) rides chunk 0. The server imports only after the length is met and the MAC verifies. Otherwise it treats the player as new or keeps its own copy.

### 2.6 Where the record physically lives

Recommendation from the storage gap report: **its own file at the homepath root next to qkey, written by new openmohaa.exe code.** Reasons, all code-read [NV]:

- The installer copies the default omconfig over the live one (installer/hzm_coop.iss:106), so a cvar record dies on reinstall. A homepath-root file is untouched by the installer.
- The config lives per game directory (maintt vs main), so a target-game switch reads a different config. The homepath root does not change with the target.
- cvar_restart deletes user-created cvars and the next frame persists the loss. It cannot reach a file at the root.
- Nine cvar loss paths are enumerated in the gap report; a root file avoids all of them.

### 2.7 Crash, upgrade and config-reset behaviour

- **Crash mid-write.** Every engine write path truncates then writes, with no temp file, no rename and no .bak (common.c:2087-2117; files.cpp:1055) [NV]. The engine has no atomic-replace helper; FS_Rename is a plain CRT rename() (files.cpp:989) [NV]. The write must be: write .tmp, fflush + _commit, MoveFileExA(REPLACE_EXISTING|WRITE_THROUGH) or ReplaceFileA, keep a .bak, and fall back to .bak on a bad checksum.
- **Two copies of the game.** No single-instance lock (sys_main_new.c:205-215) [S]. Each write must read, merge (keep the larger of each monotonic counter), then write, so a second copy cannot wipe the first.
- **Upgrade.** game.dll ships in the update manifest (manifests/manifest-1.5.3.json:19-22) [C], so a compiled-in key reaches every player. A version that does not recognise the record's major version or key id must not write it back (mirrors g_main.cpp:1301-1304) [S].
- **Config reset / qkey loss.** Nothing restores a lost qkey; a new one is generated silently and orphans every server ledger. The only real recovery is the player copying qkey and the record file together.

### 2.8 The archive store, in detail

If a cvar record is used anyway:

- A line over 1014-len(name) bytes is silently never saved; the in-session value looks fine until restart (cvar.c:1249-1262) [C].
- MAX_CVARS is 8192; exhaustion is ERR_FATAL, with a warning once the high-water index passes 6554 (cvar.c:512-536) [C]. The live archive already holds 3,951 coop_* cvars [M]. One cvar per counter would break decision F by pressuring the coop table too. **Pack into a handful of cvars; never one per counter.**
...
- **md5string** is registered (scriptthread.cpp:2002-2010) [C] but unusable as a MAC. fgame/md5.h:65 declares the word as `size_t`, so on x64 it is 64-bit, reads past its 64-byte block, and gives non-standard, non-deterministic output. A scratch harness gave "abc" two different digests on two runs, neither equal to RFC 1321 [C for the type bug; M for the runtime harness].
- **md5file** is implemented but never registered, and opens a raw OS path (scriptthread.cpp:6984) [S].
- **Script randomness** is `rand()` seeded once with Com_Milliseconds: fine for nonces, unusable as key material (g_utils.h:230-233) [S].
- **gi.CalcCRC** is a 16-bit unkeyed CRC; the import table has no hash or MAC (g_public.h:456) [P].
- **Correct MD5** (qcommon/md5.c, uint32_t) is in the exe, not exported to game.dll (qcommon/md5.c:51-216) [P]. It can be compiled into game.dll, and fixing md5.h:65 to a 32-bit word also gives correct MD5 there. md5string has no mod callers, so fixing it breaks nothing [S].
- **Asymmetric crypto is vendored** after all, correcting the tamper report's own "refuted" note: GameSpy SDK code/gamespy/common has gsCrypt.c (RSA PKCS#1 v1.5, 1024-bit sign and verify), gsLargeInt.c, gsSHA1.c, gsRC4.c. They build into gcd_common, which links the exe, not game.dll; using them in game.dll means compiling those sources into fgame [R].
- **fgame sources are GLOB_RECURSE'd**, so a new .cpp is picked up only after a cmake re-run (fgame/CMakeLists.txt:19-25) [S].

### 3.2 Engine cost of a new builtin

- **Script builtin (mp_sign / mp_verify), four sites** following compilecensus: Event object, response-table row, method declaration in scriptthread.h, implementation. A value return uses EV_RETURN + ev->AddString (md5string's pattern) (scriptthread.cpp:2096-2103, :2372; scriptthread.h:355; :7408) [P].
- **Player console event (coopprof pattern), five sites**: Event object with EV_CONSOLE, extern in player.h, response-table row, method + state member, implementation. Reached through G_ProcessClientCommand when CheckEventFlags sees EV_CONSOLE (player.cpp:328-336, :1997, :7542-7567; gamecmds.cpp:336-370) [NV].
- **The name is checked at compile time**, so the event must exist in the game.dll that compiles the script. An unknown command parse-kills the whole file (bug-298; scriptcompiler.cpp:1119-1127) [C]. Keep any new-builtin call in its own tiny .scr, so a mismatched game.dll kills only that shim.

### 3.3 The options

1. **No signing (status quo).** coop_check is dead; coop_pin1..5 are Service Record pins; coop_objKey is a HUD key name; the profile mirror's only posture is "import when the server has no record" (comments only; no import is implemented) (coop_mod/profile.scr:4-30) [P]. Fails decision D outright.
2. **Script MAC on md5string.** Broken and non-reproducible. Do not build on it.
3. **HMAC in a new game.dll builtin (recommended tier).** HMAC-SHA256 from a small vendored file, or HMAC-MD5 after fixing md5.h:65. Use a real HMAC, not H(key||msg), which MD5 and SHA-2 leave open to length extension.
4. **Asymmetric trusted-issuer tier (later).** Ed25519, or the vendored GameSpy RSA, with the private key only on HZM-run dedicated servers. Closes key extraction and listen-host minting, at the cost of key management and vendoring.

### 3.4 Threat model (honest)

Four facts shape it:

- game.dll ships to every player, so any compiled-in key belongs to everyone (publish_release.ps1:78) [C].
- Listen hosts run the signing code themselves, and the pk3 scripts are plain text.
- Identity is client-asserted cl_guid, which no server code validates (g_client.cpp:814-818) [P].
- Any server can overwrite every coop_* client cvar, and can run arbitrary client commands via `seta coop_x <cmd>` + `vstr coop_x` or a cfg trampoline (filter :173, :350-363) [C].

The vstr trampoline is a general primitive, and the tamper report's proposed fix (delete the vstr coop_* branch) fails three ways [NV LB]:

1. it kills any coop_* carrier;
2. it breaks the coop armory, which stufftexts `vstr coop_lo*` at 8 sites (loadoutpick.scr:350-621) — violating F;
3. it does not close the class: a whitelisted `exec ui/loadout/t01.cfg` runs `vstr coop_loCmt01` on a server-written cvar; 497 cfgs under allowed exec paths vstr a coop_* cvar (t01.cfg:2-3) [NV].

cgame cannot close it: cgame has no API to ask whether a token is a command, so it cannot tell a command-valued write from a multi-word data value (cg_public.h:129-136) [NV]. A complete close needs an openmohaa.exe change (inference).

What each mechanism stops:

| Attack | HMAC in game.dll + monotone merge | Trusted-issuer tier |
|---|---|---|
| Edit omconfig / type coopprof / seta | Stopped | Stopped |
| Copy another player's record to your own guid | Stopped (key bound to guid) | Stopped |
| Copy another player's qkey + record together | Not stopped | Not stopped |
| Extract the key from game.dll, mint offline | Not stopped | Stopped |
| Listen host edits scripts or farms bots | Not stopped | Stopped |
| Hostile server wipes the client record | Not stopped (can wipe, cannot forge) | Not stopped |

### 3.5 Recommended mechanism, with limits

A Tier-1 design:

- **Signing:** HMAC in two new builtins, mp_sign and mp_verify, with a constant-time compare. Keys never enter script variables.
- **Key placement:** a table of compiled-in keys indexed by key id, kept across releases so old records still verify. Per-player key = HMAC(K_kid, "hzm-mp1|" + guid), so a record copied to a different guid fails. An optional server-only cvar (never SERVERINFO) may be mixed in for closed server networks; empty means the global key.
- **Record contents:** version, key id, guid, seq, credited playtime, per-weapon kill counts, challenge counters, tag. Never store class totals or unlocks; derive both server-side.
- **Server-side plausibility checks (no key needed):** monotonic sequence, per-window kill-rate caps, a lower bot-kill cap, a kills-per-playtime ratio, no credit for telefrag / suicide / team kill, counters never below what this server last saw for that id (a per-guid high-water ledger under an MP-only save prefix). A kill credited to a weapon the player's armory cannot hold flags the record.
- **Safe failure:** on any verify failure, run session-only progress and never write back to that client. Re-sign with the current key id on a good write-back so records migrate forward.
- **Storage:** the record lives outside coop_*, delivered via the CG_ServerCommand handler, merged monotonically.

Limits, stated plainly to the user: this stops casual tampering only. Key extraction, listen-host minting and qkey cloning remain possible without a trusted-issuer tier. A hostile server can still erase (not forge) a client-held record while any trampoline exists; keep a .bak. File the trampoline as its own coop-wide security bug, independent of MP.
...
`player_killed` delegate, self = victim, 11 positional args (player.cpp:3639-3653) [C]:

1 attacker, 2 damage (float), 3 inflictor, 4 position, 5 direction, 6 normal, 7 knockback, 8 dflags, 9 meansofdeath, 10 location, 11 victim.

- Fires after KilledPlayerInDeathmatch scores (player.cpp:3563) [C]. Fires even for kills the engine does not score (telefrag, suicide, team kill, intermission), so script must re-filter [C].
- A set killhandler (m_killedLabel) returns before both scoring and the delegate, silently (player.cpp:3534-3544) [C].
- The same event also drives scriptedEvents[SE_KILL] (registerev "kill"), which allows one owner and runs with self = NULL [S].
- Subscribe from mp.scr::main after the refusal guard, once per map (delegates are wiped every restart; a double subscribe is a no-op; scriptmaster.cpp:784; scriptdelegate.cpp:118) [C].
- Registration form: `event_subscribe player_killed coop_mod/<file>.scr::<label>` with the label **unquoted**. A quoted "file::label" ScriptErrors; a bare same-file label works (gamescript.cpp:1143-1147, :1152-1165) [P]. The unquoted const-array form is code-confirmed but never executed in this mod [NV LB].
- Snapshot attacker/victim identity, dmteam, isBot, MOD, location and weapon model into locals before any wait: the args are SafePtrs and the attacker can disconnect (player.cpp:3653) [C].

### 4.2 Exact expressions

- **victim:** `local.victim`
- **attacker is world/environment:** `local.attacker == $world` (entity.cpp:2710-2715). Note: a script `damage` event on a player bypasses the world substitution, so null-check anyway (sentient.cpp:1610-1612) [P].
- **attacker is a player (human or bot):** `local.attacker != NULL && local.attacker != $world && local.attacker.classname == "Player"` [S].
- **suicide / self-inflicted:** `local.attacker == local.victim` (covers MOD 1 kill cmd, 7 fall, own grenade/rocket/explosion, trigger_hurt) [S].
- **PvP credit candidate:** attacker is a player && `local.attacker != local.victim` && `local.mod != 4` (telefrag) [C].
- **team kill:** `level.gametype >= 2 && local.attacker.dmteam == local.victim.dmteam` (only reachable with g_teamdamage 1; player.cpp:3761, :11613-11617) [P].
- **FFA:** `local.attacker.dmteam == "freeforall"` -> never a team kill.
- **bot:** `isBot local.attacker` / `isBot local.victim` (scriptthread.cpp:7364-7381) [C].
- **bullet:** `local.mod == 18 || local.mod == 19`; **shotgun:** `local.mod == 27`.
- **headshot:** `(local.mod == 18 || 19 || 27) && (local.location == 0 || 1)` (0 head, 1 helmet). Never use the EV_Killed doc numbering, which is stale (entity.cpp:195-197) [C]. Optionally add 2 (neck) if the design wants coop's definition.
- **melee:** `local.mod == 26` (MOD_BASH); location is -1 [C].
- **explosive:** `local.mod == 9 || 14 || 16 || 29`.
- **grenade:** `local.mod == 14`; **rocket:** `local.mod == 16`; **fall:** 7; **crush:** 2 or 3; **kill command:** 1.
- **weapon (hitscan/melee):** `local.w = local.attacker getactiveweap 0; if(local.w != NULL) local.wtik = local.w.model` [C].
- **weapon on turret/vehicle:** gate on `local.attacker.turret != NULL` and `local.attacker.vehicle != NULL` first, and give no weapon credit. In the torso statefile MP uses, the main-hand slot is set NULL on mount, so getactiveweap 0 is normally NULL there anyway (player_Torso.st TURRET/VEHICLE states) [P]. This corrects the earlier "holstered gun still in slot" worry.
- **grenade/rocket source:** `local.inflictor.model` — M2FGrenadeExplosion.tik (both stock grenades), bazookaexplosion_dm.tik (Bazooka and Panzerschreck splash), riflegrenadeexplosion.tik (Gewehrgranate), explosionpiatshell.tik (PIAT), MillsGrenadeExplosion.tik (Mills). The specific launcher is not recoverable for the shared cases [C].
- **killhandler guard (diagnostic):** if `local.p.killhandler != NIL`, warn — a set handler silently removes the delegate [S].
- **intermission:** the delegate still fires; progression needs its own gate (dm_manager.cpp:260-262) [C].

**MOD ordinals** (bg_public.h:479-515) [C]: 1 suicide, 2 crush, 4 telefrag, 7 falling, 9 explosion, 14 grenade, 16 rocket, 18 bullet, 19 fast_bullet, 20 vehicle, 26 bash, 27 shotgun, 28 aagun, 29 landmine.

**hitloc ordinals** (q_shared.h:1426-1449) [C]: -1 general, 0 head, 1 helmet, 2 neck, 3 torso_upper ... A helmet hit on a victim with no helmet is folded to head before Killed. A handler never sees -2 (ArmorDamage returns on HITLOC_MISS).

### 4.3 Kill paths and what the handler receives

- **Bullet:** attacker = gun owner (or a VehicleTurretGun's remote owner when the turret has no direct owner), inflictor = world, MOD from tik, location = trace hitloc (weaputils.cpp:2966-2977; weapon.cpp:2209-2214) [C].
- **Melee:** attacker = inflictor = attacker, MOD bash, location -1 [C].
- **Projectile direct hit:** attacker = owner (world if gone), inflictor = the Projectile, MOD from projectile tik, location -1 [C].
- **Splash:** attacker = owner, inflictor = a fresh Explosion (model = explosionmodel), MOD from the explosion tik, location -1. Bazooka/Panzerschreck/PIAT splash report MOD_EXPLOSION (9), not rocket [C].
- **Owner disconnected / changed team mid-flight:** projectile removed, no damage (weaputils.cpp:1455-1490) [S].
- **Owner dead:** credited to owner; only weap nulled [S].
- **kill / fall / trigger_hurt / world crush / telefrag:** attacker==victim except world crush (world) and telefrag (arriving ent) [S].
- **Fixed MG42 nest:** owner is the using player, so the kill credits that player while the main hand is not the gun — the turret gate handles it [C].
- **Quick-draw:** the pistol is in WEAPON_MAIN, so getactiveweap 0 correctly returns the pistol (player.cpp:16573-16574) [S].

### 4.4 The engine prerequisite

Restated because it is the gate on all of the above: PvP damage is dropped today (section 1.3, risk 1). Fix Sentient::ArmorDamage with a gate proven inert for coop. Candidate: treat two Players on different DM teams (or FFA, or g_teamdamage) as different teams. Open question in 9(c).

### 4.5 In-game test list (dedicated server, second client)

Run on a dedicated server with a remote client, ideally with sv_floodProtect 1. Listen-host results do not count.

- **P0 gate:** shoot an enemy bot; health must drop and it must die with an obituary. If not, the m_Team filter is live and nothing below passes.
- **P0b:** with g_teamdamage 0, a teammate takes no damage.
- **P1:** `event_subscribe player_killed coop_mod/<stub>.scr::onKill` from mp.scr::main prints no Script Error; the stub fires on a death.
- **P2:** the stub prints all 11 args; arg1 = killer entnum, arg11 = victim = self, arg9 = MOD, arg10 = location.
- **P3:** after a same-map restart and a map change, the next kill still prints.
- **P4:** rifle headshot -> location 0/1, MOD 18/19; body -> 3-6.
- **P5:** rifle-butt -> MOD 26, location -1, getactiveweap 0 .model = the rifle.
- **P6:** grenade, switch to rifle, kill with the blast -> MOD 14, inflictor.model = M2FGrenadeExplosion.tik, getactiveweap 0 = rifle.
- **P7:** `kill` (after 5 s) -> attacker==victim, MOD 1; fall death -> attacker==victim, MOD 7.
- **P8:** isBot(attacker) is 1 when the bot kills you, 0 when you kill it.
- **P9:** on a coop map, the stub never prints on a coop death.

---

## 5. Class and weapon mapping

### 5.1 Why the map must be a table

Three taxonomies disagree, so class cannot be read from the engine [NV/C]:

- Engine class bits: PISTOL, RIFLE, SMG, MG, GRENADE, HEAVY. No SNIPER bit, no SHOTGUN bit (bg_public.h:385-396) [C].
- Tik weapontype: shotgun and the rockets are both 'heavy'; StG44/BAR/FG42/DP-28 are 'mg' but StG44 Scoped is 'rifle' (various tiks) [C].
- Armory tab and challenge category disagree again (StG44 is 'rifles' in challenges, 'heavy' in the roster) [NV].

So: decide the class at kill time from a generated exact-match table keyed on the lowercase model path, and freeze it into the record. Do not re-bucket later; a class-map change then only affects future kills.

### 5.2 The recipe (kill -> class tab, base weapon id)

0. Hook player_killed; read args 1, 3, 9, 11 (section 4).
1. Gates (in order): drop if attacker is not a Player; attacker==victim; MOD 4; team kill; MOD 9/20/2/3. Bots pass (E).
2. Emplacement gate: if attacker.turret or attacker.vehicle is non-NULL, give no weapon/class credit (optionally an "emplacement" stat).
3. Resolve the model by MOD: 18/19/27/26 -> `attacker getactiveweap 0`.model; 16 -> active weapon, else inflictor.model (PIAT unique; Bazooka vs Panzerschreck by attacker side); 14 -> inflictor.model (Mills and Gewehrgranate unique; M2 vs Stielhandgranate and Bomba vs F1 by side or a throw latch); 29 -> no credit.
4. Look the model up in the generated exact-match table (roster gives + coop_skinGive variants mapped to base + stock grenade aliases). Keys lowercased. No substring matching.
5. Class tab = the killing tik's own roster row (a scoped Garand kill is SNIPER). Base weapon id = roster id after the skin strip. Family id optional.
6. Class ladder bumps by class regardless of team (C). Per-weapon ladder bumps by roster id.

### 5.3 The two armories

The roster is 74 ids, tabs 0 rifle / 1 sniper / 2 smg / 3 heavy / 4 pistol / 5 grenade; tab and give match loadout_weapons.tsv for all 74 [NV]. Proposed side split: 42 Allied, 32 Axis, no overlap; RIFLE 8/4, SNIPER 7/5, SMG 8/5, HEAVY 7/6, PISTOL 9/9, NADES 3/3 [P, count-level]. Allied = US/UK/USSR; Axis = German/Italian/Japanese.

Full per-side tables (roster id -> tik -> tab -> MP class, with coop family for variants) are in the class-mapping report's inventories. Key points for design:

- **Shotgun (44)** is a single tik and skin set for both sides; one base id. It is the only row the side filter must allow for both teams. In the current roster split it sits on the Allied side, which conflicts with G's "on both armories" — tile 44 needs a second target (this contradicts the earlier plan's "no weapon on both sides").
- **StG44 Scoped (38):** engine says 'rifle'; design decides MG vs SNIPER.
- **thompsonsmg_gold (71):** treat as a skin of 24, not a gun.
- **28 roster tiks exist only in maintt/zzzzz_xw_weapons.pk3**, which is untracked and outside build.ps1 [P]. A generator that sweeps only the repo misses them, and a server without that pak cannot give or attribute them. (4 of the pack's 32 tiks — g43sniper, tt33, tt33silenced, welrod — are git-tracked and shipped in the mod pak, which wins; 28 are xw-only.)

### 5.4 Edge cases

- **Emplacements:** MOD 18, inflictor world, main hand NULL after mount. No class credit.
- **Grenades:** MOD 14; the attacker may have switched or died; explosion model separates Mills and the rifle grenade but not M2/Stielhandgranate or Bomba/F1. Grenade class is never ambiguous; the specific grenade needs side or a throw latch.
- **Picked-up grenades:** the engine waives the class check for grenades, so an Allied player can throw a Stielhandgranate; side-based disambiguation then miscredits. Only a throw-time latch is exact.
- **Rockets:** MOD 16; Bazooka and Panzerschreck share projectile and explosion in DM; prefer the active weapon, fall back to side.
- **Picked-up enemy primaries:** mostly blocked by the engine (a player with a primary cannot take a different-class primary; same class gives ammo only). When acquired, the model is the enemy tik, so the class credit is correct under C; whether the per-weapon counter should move is a policy choice (weapon.cpp:3959-4077) [C].
- **Non-roster stock-kit items:** kar98_mortar, the *_lite rifles, minedetectors, Russian F1, RDG-1, bombabreda — decide whether to map to a base id or leave out of MP.
- **coop chal_widFromModel cannot be copied or called (F, clause 10), and misattributes anyway:** no id for carbine, silenced Colt, FG42 or any BAR skin; Breda smoke -> 'breda'; Gewehrgranate and kar98_g98 -> 'kar98'; scoped/silenced fold across tabs [P].

---

## 6. Match events per gametype

Gametypes: 1 FFA, 2 TDM, 3 Round TDM, 4 OBJ, 5 TOW, 6 LIB.

| Event | Gametypes | Hook / derivation | Reliability | Engine work? |
|---|---|---|---|---|
| Kill (base feed) | 1-6 | event_subscribe player_killed; re-subscribe each level load; re-filter | HIGH | No |
| Headshot | 1-6 | arg 10 in {0,1,2} | HIGH (hitscan) | No |
| Kill streak | 1-6 | per-attacker counter, reset on death and restart | HIGH | No |
| Multikill | 1-6 | per-attacker last-kill level.time window | HIGH | No |
| First blood | 1-6 | first valid kill after roundstart (3-6) or map load (1-2) | HIGH | No |
| Revenge | 1-6 | store victim's lastKiller by client number | HIGH within map | No |
| Assist | 1-6 | player_damaged accumulates damage per (victim, attacker) | MEDIUM-HIGH | No |
| Round win/loss/draw | 3-6 only | level waittill allieswin / axiswin / draw | HIGH | No (optional dm_teamwin delegate) |
| Match end | 1-6 | none; SE_INTERMISSION never triggered | LOW | YES for reliability |
| OBJ bomb plant/defuse/destroy | 4 | poll level.bombs_planted / targets_destroyed (team); useheld timing (per-player) | team HIGH / player MEDIUM | YES for exact attribution |
| TOW capture | 5 | poll tow_* cvars (team); getkills +2/+5 on 4 of 7 maps (per-player) | team HIGH / player MEDIUM or NONE | YES for exact attribution |
| Liberation rescue | 6 | level.axisLiberator / alliesLiberator + injail transitions | HIGH | No |
| Liberation capture | 6 | a valid kill in gt 6 | MEDIUM-HIGH | No |
| Playtime | 1-6 | sample every N s, flush at disconnect | HIGH (+-N s) | No |
| Survival time | 1-6 | spawn to death via level.time | HIGH | No |
| Accuracy / shots fired | 1-6 | not observable (all engine counters SP-gated, no getter) | NONE | YES |
| Round start | 3-6 | level waittill roundstart | HIGH | No |

Key facts behind the table [NV unless noted]:

- The six script delegates are the only ones in fgame: player_connected, player_disconnecting, player_spawned, player_damaged, player_killed, player_textMessage. No round/team-win/intermission/objective/TOW/jail delegate exists [NV].
- allieswin/axiswin/draw fire 2 s after teamwin and ~3 s before the restart; roundstart never fires in FFA/TDM (waiting on it there hangs forever) [NV].
- Match end is silent everywhere; SE_INTERMISSION has a name but no Trigger site [C].
- During MP intermission, script `wait` timers do not advance (~15 s), because ScriptMaster::SetTime is skipped, though an event processed in L_ProcessPendingEvents can still run a thread [P].
- Overriding global/obj_dm.scr to capture the bomb planter would alter coop map m3l1b (which threads bomb_thinker) and would miss the inline copies in bizertefort/ship_lib — so do not (maps/m3l1b.scr:2128-2129) [NV].
- Round transitions run a full `restart` that keeps the reliable queue but deletes entities and vars (section 3, risk 3) [C].
...
One `chal_def` row = 7 fields (id, cat, title, desc, stat, target, reward) in parallel level arrays, plus a cid->index map and a per-stat reverse index. Adding a challenge is one line (challenges.scr:732-752) [NV]. There is no tier concept: base/elite/variant are separate rows on one stat with bigger targets [P]. Variant targets follow variant_challenges.py's curve `B*(3+k^1.3)` rounded to 25, with a 750 cap; once a gun's tail passes 750 the whole set is rescaled onto [first, 750], so adding one variant moves the others' targets [P/NV LB].

Lifecycle: DEFINE (chal_init, one frame) -> IDENTIFY (xp_identify, waits <=5 s for guid) -> LOAD (chal_ensure, lazy) -> WRITE (chal_bump -> chal_apply, clamped) -> GRANT (chal_grant: append unlock, toast, save, medals, padlock refresh) -> RANK (re-derived from XP) -> SAVE (30 s / 60 s / on grant) [NV].

Two unlock truths: guns and cosmetics check the grant string; variants and mastery check coop_chalD_ done flags re-derived at load as progress >= current target [C]. **For MP, store counters and derive unlocks — smaller and easier to verify.**

### 7.2 Unit of storage and latch policy (from the sixth gap report)

- **Store both class counters and per-source counters, as independent increments at kill time.** Class counters drive gun ladders; per-source counters drive variant/skin ladders. Never compute class totals from weapon counters — the stock kit hands out non-roster guns, coop's wid merges rifle/sniper families and has no key for carbine/FG42/silenced Colt/grenades, and class has no single answer [NV].
- **Store raw, unclamped counters** (the opposite of coop's clamp), so a later target or curve change can always be recomputed.
- **Unlocks = latched append-only set + derive(current counters, current tables), where the derivation may only grant, never revoke.** Re-deriving alone would re-lock gear whenever a target moves (variant_challenges.py rescale) [NV LB]. Needs the user's yes (9a).
- **Invariant that survives every version:** each kill adds exactly one class kill and at most one weapon kill, so the sum of weapon counters can never exceed the sum of class counters — a cheap sanity check, not the integrity mechanism (the MAC is).

### 7.3 What is generated vs hand-written

Generated (re-runnable into new MP files with an MP table): ui/coop_sr.urc + 29 baked page textures + pin cfgs + SR stamp (gen_service_record.py, blocking in build.ps1); mvchal_gen.scr (variant_challenges.py); unlockreq_gen.scr (armory_unlocks.py); cosmetic hover text (gen_cos_reqs.py); ui/loadout req/p/t/lktab cfgs (gen_loadout.py); reqmv cfgs (wire_mv2.py, already stale) [NV].

Hand-written: challenges.scr (368 rows + chal_widFromModel + hooks), xp.scr rank table, loadout_weapons.tsv req text, the 188 stat-writer sites, medals.scr, and the engine CL_PinToggle_f / CL_SyncSR_f with hard-coded coop cvar names [NV].

Only gen_service_record, unlock_audit and check_mp_isolation are blocking in build.ps1; check_challenges is non-blocking; the rest are hand-run and one (reqmv text) is already stale [NV].

### 7.4 PvP equivalents

- **Direct:** the five weapon categories (per-weapon `wpn_<wid>` and class ladders) map straight onto C.
- **Mostly:** combat (kills, headshot, melee, grenade, rocket, longshot, wallbang — the engine sets coop_wallbang on any Player-owned bullet hit, so it is reusable [S]).
- **Weak/none:** fireteam (revive/ammo/airstrike are coop mechanics; PvP needs assists/objectives/round wins as new writers), vehicles (only on vehicle maps), axis (faction-of-victim; C makes class team-agnostic anyway), campaign/discovery/finishes (SP-map keyed; MP analogue is gametype- or match-scoped).

### 7.5 UI surfaces and their costs

- **Completion toast:** ihuddraw slots 76-78, 0 client cvars, 4 stufftexts + audio.
- **Progress popup (pinned only):** slots 84-87.
- **In-game SR panel:** slots 150-174, ~5 ihuddraw/row x 18 rows, redraw 0.5 s; network messages, not reliable commands.
- **Offline Service Record:** 3 archived cvars per row (~1,780 for 445 rows), first export up to 1,335 reliable commands per player against a 1024 ring (safe only because paced), plus 486 MB of baked page textures. Join-time export was removed after it dropped clients (bug-670).
- **Pins HUD:** up to 16 stufftexts per player every 3 s, not diff-gated.
- **Gun padlocks:** 2 archived cvars per gun, diff-only. Cosmetic padlocks: 1 per armory page.
- **Requirement hovers:** 0 server commands (client-local exec of static cfgs) [NV].

**MP sizing (1:1 copy across both armories, per-gun skins gated):** roughly 1,143 stufftexts, ~7 s at 8-per-frame and sv_fps 20 [NV]. The binding limit is not the 1024 ring (paced) but MAX_CVARS: archiving ~1,200 more cvars would be re-created in every coop session too, pressuring the 3,951 coop_* cvars already live and risking the ERR_FATAL that hit before (bug-1582) — which breaks F [C/M]. So: use session `set`, not `seta`; push once per map after verification, joined armory only; collapse per-gun skin locks into one mask cvar per gun that execs a generated cfg.

### 7.6 Coop namespaces MP must never touch, and isolation clauses to add

**Never read or write** (representative, from the challenge-template report): save files chal_/unlocks_/pins_/pend_/medals_/xp_<id>.dat; session cvars coop_chal_/unlocks_/pins_/pend_/medals_/xp_<id>; player flags coop_chalP_*/coop_chalD_*/coop_pin*/coop_xp_*/coop_medal*; level vars coop_chal_*/coop_xp_*/coop_unlockReq*; client cvars coop_uiB/N/D/P/M*, coop_srP*, coop_pin*, coop_cp*, coop_loLk*, coop_loReq*, coop_loCosReq*; commands coop_pintoggle/coop_srsync; HUD slots 62-78, 84-87, 117-126, 150-174; name-bus indices 34 and 47. Also dm_playermodel, dm_playergermanmodel, coop_loSkin (section for character gear below).

**Clauses to add to check_mp_isolation.py** (today clause 10 scans only mp.scr; clause 7 knows only coop_mp(a<d>|x<d>|FreeKit|LockLoadout); check_challenges' writer scan covers the whole tree) [C]:

- Enumerate MP files by glob and run the call/leak scans over all of them, not just mp.scr.
- Forbid MP files referencing challenges/xp/medals/loadoutpick/loadoutskins/helmet/gloves/lobby/events/eventsystem labels, and forbid setting killhandler.
- Forbid MP reading/writing coop save prefixes and coop session/flag/client-cvar/HUD-slot namespaces.
- Give MP its own writer verb names, and exclude MP files from check_challenges' tree-wide writer scan.
- Add runtime `if(level.coop_mpRun != 1){ end }` guards to chal_bump, chal_team_bump, cc_award_clean, chal_add_unlock and xp_award, because mp.scr sets coop_isActive=1 and runs coop per-life code on MP maps.
- Require MP record cvar names to start coop_ (filter rule) but match no coop regex — widen clause 7 so the chosen MP prefix cannot appear in coop scripts.

### 7.7 Character gear without the shared key (from the second gap report)

MP wears the coop skin today because every MP spawn re-reads dm_playermodel and InitModel applies it, and mp.scr does no model work (player.cpp:2815-2871; mp.scr:218-251) [NV].

- **Never read or write dm_playermodel / dm_playergermanmodel in MP.** Treat InitModel's pick as untrusted and overwrite it every spawn and every team switch (a team switch re-runs InitModel: player.cpp:11483-11485) [NV].
- **Choose the body only from a server-held MP roster per team**, applied with the script `model` command (no prefix check), then re-apply headgear and glove bits (setModel wipes s.surfaces) (entity.cpp:2034-2040) [NV].
- **Timing:** re-model synchronously inside a player_spawned handler (fires in the spawn frame, before the frame ends) for no leak window; the 0.5 s poll leaves the coop skin visible for up to half a second; a fail-closed engine override is the strongest.
- **Voice nationality, stock kit and weapon menu still come from the userinfo key** (player.cpp:10468-10513, 12201-12225) [NV]. Italian Axis skins keep the German voice unless the key is german/it/sc-prefixed.
- **Shipped assets census** [M]: Allied 135 skins (coop's roster), 23 nation headgear props + 2 glasses + Wool Cap, 7 gloves. Axis 25 stock bodies (21 German, 4 Italian), 19 nation props + 2 glasses, **zero gloves** (no Axis tik carries glove skins), **zero hatless twins**. **Japanese: zero player models in any pak** — the plan's "88 Axis models" counts HRRTM, which the mod does not ship. Japanese character gear cannot be built from existing assets.
- **Add isolation clauses** forbidding MP mention of dm_playermodel, dm_playergermanmodel, coop_loSkin, coop_loChar, coop_armorySkin, coop_helmetIdx, coop_gloveIdx, and forbidding MP `exec ui/loadout/*` or push coop_loadout.
...
- Engine bots are compiled into the deployed game.dll (fgame.vcxproj:617, :675-679; "isBot" and "bot_manualmove" in G:/mohaa-gl2/game.dll) [M].
- Each bot is a real Player entity (`new Player`, targetname "player"), so $player, player_killed and the whole Player path include bots (g_bot.cpp:158-170; player.cpp:2508) [NV]. Bots have no client, no svs.clients slot, no cl_guid and no archived cvars [NV].
- The only reliable test is `isBot <ent>` (SVF_BOT). NA_BOT is never assigned; ip "localhost" also matches the listen host; coop_guid is NIL/"" (scriptthread.cpp:7364-7381; g_client.cpp:852) [NV/C].
- The bot entity is rebuilt on every map; bots persist across map changes in any non-SP gametype, coop included (g_bot.cpp:573-634, :624) [NV]. So the mod must never set the bot cvars.
- **stufftext on a bot is not a no-op:** Player::EventStuffText runs the text server-side as the bot's console command, unfiltered, if the first token names an EV_CONSOLE event; unknown commands leak the parse buffer (player.cpp:12372-12384; playerbot.cpp:279-330) [NV LB]. mp.scr already leaks a buffer on every bot spawn (mp.scr:252-253) [NV].
- Bots self-deploy (primarydmweapon auto + auto_join_team + a fire toggle), so an armory menu cannot stall them, and a gate that waits for a client-side commit would stall them forever (playerbot.cpp:128-166) [NV].
- Only the server console/rcon/cfg can add bots; a self-hoster can add them and freeze them with bot_manualmove 1 (gamecmds.cpp:271-275; playerbot.cpp:128-131) [NV].
- The mod and its harness have never used engine bots — the "bots" in buglog are real openmohaa.exe clients driven by coop_botInput [S].

### 8.2 Rules for the flow

- Classify with `isBot <ent>` at the moment of use, never cached across maps, never by netname/entnum/ip/guid.
- Bots get no client pipeline: skip stufftext, cvar pushes, ui/coop_* exec, menus, profile challenge/response and HUD toasts. Add an isBot early-out before the client-only parts of mp_deploy/mp_onSpawn, keeping server-side work such as `health 100`.
- The spawn path must work with no client action, and must not ban all classes via dmflags (the engine reverts that on the first team join: player.cpp:18469-18476) [NV].
- Give bots the engine stock nationality kit or a fixed server kit, re-applied each life (the class re-rolls on death).
- The mod must never set sv_maxbots, sv_sharedbots, sv_numbots, sv_minPlayers or bot_manualmove; add an isolation clause forbidding those cvars in mod files.

### 8.3 Rules for progress

- Bots get no progression I/O: no id, no file, no cvar, no server mirror, and never the "n_"+netname fallback.
- Kill-credit rule: credit when attacker is a Player, isBot(attacker)==0, attacker != victim, MOD != 4, and (team gametypes) different dmteam. The victim may be human or bot (E). Do nothing when the attacker is a bot.
- Because a self-hoster can farm frozen bots for server-attested kills, portable progress needs an explicit anti-farm policy in the tamper layer (a cap, a weighting, or eligibility such as bot_manualmove==0). The server can attest kills; it cannot attest that the server is honest.
